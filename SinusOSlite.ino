...
const unsigned char PROGMEM factory_reset_icon[] = {
  ...
  0x01, 0x00, 0x80, 0x00, 0x80, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
void displayResetWarning(int secondsLeft) {
  ...
  String countText = (config.language == 0) ? "Ust. fabr za: " + String(secondsLeft) + "s" : "Wipe data in: " + String(secondsLeft) + "s";
...
void setupWiFi() {
  if (config.configured && strlen(config.ssid) > 0) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      apMode = false;
      MDNS.begin("sinusoslite");
      displayMessage(getText(2), "sinusoslite.local", "v: 1.5");
    } else {
...
void loadConfig() {
  ...
  if (config.initialized != true || isnan(config.latitude)) {
    ...
    memset(config.ssid, 0, 32);
    memset(config.password, 0, 64);
    String("Wodzisław Śląski").toCharArray(config.cityName, 32);
    saveConfig();
  }
...
String urlEncode(String str) {
  ...
  for (int i = 0; i < str.length(); i++) {
    ...
    else { encodedString += String('%'); if ((unsigned char)c < 16) encodedString += "0"; encodedString += String((unsigned char)c, HEX); }
  }
